...
// Supabase'i güvenli bir şekilde başlat
let supabase;
try {
    if (window.supabase) {
        ...
        console.error("Supabase SDK not found! Check your internet connection or script tag.");
    }
} catch (e) {
...
async function startSupabaseListeners() {
    if (!supabase) {
        console.warn("Supabase client not available. Listeners not started.");
        return;
    }
    ...
    try {
        ...
        if (gameError) console.error("Error fetching game state:", gameError);
    ...
    } catch (err) { console.error("Supabase error in startSupabaseListeners:", err); }
}
...
function updateGameUI() {
    ...
    if (turnEl) {
        if (checkWinner()) {
            ...
            const winner = board[pattern[0]] === "X" ? "Anıl" : "Camila";
...
function update() {
    ...
    const milanTimeEl = document.getElementById("milan-time");
    const bogotaTimeEl = document.getElementById("bogota-time");
    if (milanTimeEl) milanTimeEl.innerText = milan.getHours() + ":" + String(milan.getMinutes()).padStart(2, '0');
    if (bogotaTimeEl) bogotaTimeEl.innerText = bogota.getHours() + ":" + String(bogota.getMinutes()).padStart(2, '0');
    ...
    if (msgEl) msgEl.innerText = messages[dayDiff] || "Contigo, siempre. 🤍";
...
function updateSelfieUI() {
    ...
    if (anilImg && anilPlaceholder) {
        if (anilPhoto) { anilImg.src = anilPhoto; anilImg.classList.add("active"); anilPlaceholder.style.display = "none"; }
        else { anilImg.classList.remove("active"); anilPlaceholder.style.display = "flex"; }
    }

    if (camilaImg && camilaPlaceholder) {
        if (camilaPhoto) { camilaImg.src = camilaPhoto; camilaImg.classList.add("active"); camilaPlaceholder.style.display = "none"; }
        else { camilaImg.classList.remove("active"); camilaPlaceholder.style.display = "flex"; }
    }

    if (anilImg && camilaImg) {
        if (anilPhoto && camilaPhoto) {
            anilImg.classList.remove("blurred"); camilaImg.classList.remove("blurred");
        } else {
            if (currentUser === "Anıl") {
    ...
    reader.onload = async function(event) {
        ...
        img.onload = async function() {
            ...
            if (supabase) {
                ...
                await supabase.from('selfies').upsert(updateData, { onConflict: 'day' });
            }
        };
    };
